  ...
  describe ".handle_buffer/3" do
    ...
    context "queue is empty" do
      ...
      context "frame is contained in the buffer" do
        ...
        it "shoud return tuple with :send_buffer" do
          {result_atom, _decoded, _new_state} = described_module.handle_buffer(caps, buffer, state)
          expect(result_atom).to eq(:send_buffer)
        end

        it "should return non empty result" do
          {_result_atom, {caps, decoded}, _new_state} = described_module.handle_buffer(caps, buffer, state)
          expect(decoded).to be_bitstring
          expect byte_size(decoded) |> to(be :>, 0)
        end

        it "should return state with new queue" do
          {_result_atom, _nbuf, %{queue: new_queue}} = described_module.handle_buffer(caps, buffer, state)
    ...
    context "queue is not empty" do
      ...
      context "frame is contained in the queue and buffer" do 
        let :buffer, do: <<0, 0, 0, 76, 65, 77, 69, 51, 46, 57, 54, 46, 49, 85, 255, 243, 20, 196, 11, 0, 0, 3, 72, 0, 0, 0, 0, 85, 85, 85, 85, 85, 85, 85, 85, 85, 85, 85, 255, 243, 20, 196, 22, 0, 0, 3, 72, 0, 0, 0, 0, 85, 85, 85, 85, 85, 85, 85, 85, 85, 85, 85, 255, 243, 20, 196, 33, 0, 0, 3, 72, 0, 0, 0, 0, 85, 85, 85, 85, 85, 85, 85, 85, 85, 85, 85 >> 
      ...
      context "frame is not contained in the queue and buffer" do
        ...
        pending "should return an ok result" 
        #  expect(described_module.handle_buffer(caps, buffer, state)).to be_ok_result
        #end

        pending "should return new queue containing old queue concatenated with buffer"
        #   {_result_atom, %{queue: new_queue}} = described_module.handle_buffer(caps, buffer, state)
